...
public class RovFile {
     ...
     */
    public void match(){
        Vector<PeptideIdentification> lIds = iMascotIdentificationFile.extractDatfilePeptideIdentification(iThreshold);
            ...
            for(int j = 0; j<lIds.size(); j ++){
                for(int p =0; p<lDistillerPeptides.size(); p++){
                    ...
                    for(int k = 0; k<lRatioGroup.getDatfileQueries().length; k ++){
                        if(lDistillerPeptides.get(p).getQuery() == lRatioGroup.getDatfileQueries()[k] && lDistillerPeptides.get(p).getQuery() == lIds.get(j).getDatfile_query() && lRatioGroup.getPeptideSequence().equalsIgnoreCase(lIds.get(j).getSequence())){
                            lRatioGroup.addIdentification(lIds.get(j), lDistillerPeptides.get(p).getComposition());
                        }
                    }
                }
            }
