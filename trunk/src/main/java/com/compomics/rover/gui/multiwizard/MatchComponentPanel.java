...
public class MatchComponentPanel implements WizardPanel {
    ...
    private JButton renameRatioComponentsAndButton;
    ...
    public void nextRatio() {
        if (iComponentTypeCounter < iCollectionsComponents.get(iIndexCounter).length - 1) {
            ...
            if (iIndexCounter == iCollectionsComponents.size()) {
                ...
                renameRatioComponentsAndButton.setVisible(true);
                lblSet.setText("All set");
                lblLabel.setText("Click \"Rename ratio ... \"");
                ...
                lblSet.setText("Set " + (iIndexCounter + 1) + "/" + iCollectionsComponents.size());
                lblComponentLabelCount.setText("Component " + (iComponentTypeCounter + 1) + "/" + iCollectionsComponents.get(iIndexCounter).length);
                lblLabel.setText(iCollectionsComponents.get(iIndexCounter)[iComponentTypeCounter]);
            }
        }
        ...
        linkToANewlyButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (lName.equalsIgnoreCase("Newly created component type name")) {
                    ...
                    JOptionPane.showMessageDialog(iParent, "Set a valid name for the newly created component type!", "ERROR", JOptionPane.ERROR_MESSAGE);
        ...
        linkToSelectedMergedButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (lMerged != null) {
                    ...
                    JOptionPane.showMessageDialog(iParent, "No component was selected!", "ERROR", JOptionPane.ERROR_MESSAGE);
                }
            }
        });
        renameRatioComponentsAndButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                        ...
                        for (int k = 0; k < lRatioGroup.getNumberOfRatios(); k++) {
                            ...
                            if (lMustBeInverted) {
                                ...
                                if (lRatioGroup.getParentCollection().getRoverSource() == RoverSource.MAX_QUANT || lRatioGroup.getParentCollection().getRoverSource() == RoverSource.MAX_QUANT_NO_SIGN) {
                                    ...
                                    double lTempRatioNorm = lRatio.getRatio(false);
                                    //inverted it
                                    lTempRatioNorm = Math.pow(lTempRatioNorm, -1);
                                    //set it as a log 2 value
                                    MaxQuantRatio lMaxRatio = (MaxQuantRatio) lRatio;
                                    lMaxRatio.setRecalculatedNormRatio(Math.log(lTempRatio) / Math.log(2));
                                }
                            }
    ...
    public void construct() {
        ...
        lblSet.setText("Set 1/" + iCollectionsComponents.size());
        ...
        renameRatioComponentsAndButton.setVisible(false);
    ...
    private void $$$setupUI$$$() {
        ...
        gbc.anchor = GridBagConstraints.WEST;
        gbc.insets = new Insets(5, 5, 5, 5);
        jpanContent.add(lblComponentLabelCount, gbc);
        lblLabel = new JLabel();
        ...
        renameRatioComponentsAndButton = new JButton();
        renameRatioComponentsAndButton.setText("Rename ratio components and types");
        gbc = new GridBagConstraints();
        gbc.gridx = 0;
        gbc.gridy = 7;
        gbc.gridwidth = 2;
        gbc.fill = GridBagConstraints.HORIZONTAL;
        gbc.insets = new Insets(5, 5, 5, 5);
        jpanContent.add(renameRatioComponentsAndButton, gbc);
